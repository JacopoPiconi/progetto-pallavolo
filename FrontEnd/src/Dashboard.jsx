    ...
    <div style={containerStyle}>
      <nav style={navStyle}>
        ...
        <div style={{ display: 'flex', gap: '15px' }}>
          <button onClick={onBackClick} style={backBtnStyle}>Home</button>
          <button onClick={onLoginClick} style={loginBtnStyle}>Accedi</button>
        </div>
      </nav>
      ...
      <div style={{ padding: '30px 5%' }}>
        ...
        <div style={statsGrid}>
          {['Squadre Monitorate', 'Partite Programmate', 'Rating Medio', 'Leghe Attive'].map((label, i) => (
            <div key={i} style={kpiCard}>
              <p style={{color: '#64748b', margin: 0}}>{label}</p>
              <h3 style={{fontSize: '1.8rem', margin: '5px 0'}}>{[6, 2, 1630, 2][i]}</h3>
            </div>
          ))}
        </div>
...
const statsGrid = { display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '20px', margin: '25px 0' };
const kpiCard = { background: 'white', padding: '20px', borderRadius: '15px', border: '1px solid #e2e8f0' };
const loginBtnStyle = { padding: '10px 20px', backgroundColor: '#fbc02d', color: '#1a237e', border: 'none', borderRadius: '50px', fontWeight: 'bold', cursor: 'pointer' };
const backBtnStyle = { padding: '10px 20px', background: 'transparent', color: 'white', border: '1px solid white', borderRadius: '50px', cursor: 'pointer' };
